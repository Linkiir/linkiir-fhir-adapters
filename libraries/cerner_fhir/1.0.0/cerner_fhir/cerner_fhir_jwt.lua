-- ---------------------------------------------------------------------------
-- cerner_fhir_jwt - signs JSON Web Tokens
--
-- Cerner authenticates backend services with a signed JWT rather than a shared
...
--       algo    = 'RS384',
--       key     = PrivateKeyPem,
--       header  = { alg = 'RS384', typ = 'JWT', kid = 'my-key-id' },
...
-- Cerner accepts RS384 assertions. The rest are here so this module stays
-- usable as a plain JWT signer elsewhere.
--
...
-- produces tokens that Cerner rejects, so this transform is required, not
-- cosmetic.
...
--   T.header  - JWT header table (include kid for Cerner)
...
function M.sign(T)
   ...
   if not Algorithm then
      error("cerner_fhir_jwt.sign: unsupported algorithm '" .. tostring(T.algo) .. "'")
   end
   if not T.key or T.key == '' then
      error('cerner_fhir_jwt.sign: missing signing key')
   end
