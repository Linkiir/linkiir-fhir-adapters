-- ---------------------------------------------------------------------------
-- epic_fhir_http - the request path every FHIR call goes through
--
-- Attaches the bearer token, builds the resource URL, sends the request, and
-- turns whatever Epic returns into one predictable pair:
--
--    Result        - the parsed resource or Bundle
--    nil, Err      - Err is { code=, message= } plus optional context
--
-- Two Epic behaviours are worth knowing, because both look like success at the
-- HTTP layer and are reported as failures here:
--
--   * An OperationOutcome resource can arrive with HTTP 200. Epic uses it for
--     business-level problems such as a search with no usable parameters.
...
-- Interpret a response with no body. A 2xx here is Epic's create success.
local function readEmptyBody(Response)
   ...
   return nil, {
      ...
      message = 'Epic returned HTTP ' .. tostring(Response.code) .. ' with an empty body',
   }
end
...
local function readBody(Response)
   ...
   if not Ok then
      -- Requests pin Accept to application/fhir+json, so a body that is not
      -- JSON is a genuine fault rather than an alternate format to handle.
      return nil, {
         code      = 'PARSE_ERROR',
         message   = 'Epic response was not valid JSON',
   ...
   if isOperationOutcome(Parsed) then
      return nil, {
         ...
         message   = outcomeText(Parsed) or 'Epic returned an OperationOutcome',
         http_code = Response.code,
         outcome   = Parsed,
      }
   end
   ...
   if Response.code < 200 or Response.code >= 300 then
      return nil, {
         code      = 'HTTP_' .. tostring(Response.code),
         message   = 'Epic returned HTTP ' .. tostring(Response.code),
         http_code = Response.code,
         body      = Parsed,
      }
   end
...
--   T.api        - path below /api/FHIR/<version>/, e.g. 'Patient/eXYZ123'
...
function M.request(Client, T)
   ...
   if not SendRequest then
      error("epic_fhir_http.request: unsupported HTTP method '" .. Method .. "'")
   end
   ...
   local Request = {
      url       = Client.base_url .. 'api/FHIR/' .. Client.version .. '/' .. tostring(T.api),
      headers   = Headers,
   ...
   linkiir.log.debug('epic_fhir ' .. Method:upper() .. ' ' .. Request.url)
