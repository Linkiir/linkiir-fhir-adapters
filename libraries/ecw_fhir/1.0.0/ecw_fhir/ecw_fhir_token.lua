-- ---------------------------------------------------------------------------
-- ecw_fhir_token - access token cache
--
-- eCW access tokens are valid for a limited time, so re-authenticating on
...
-- credential out of the project tree and out of git.
-- ---------------------------------------------------------------------------

local M = {}

-- Keyed by "<client id>@<auth url>" so a node configured against more than one
-- eCW environment cannot hand the wrong token to the wrong endpoint.
local Cache = {}
