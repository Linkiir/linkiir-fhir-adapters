-- ---------------------------------------------------------------------------
-- ecw_fhir_http - the request path every FHIR call goes through
--
-- Attaches the bearer token, builds the resource URL, sends the request, and
-- turns whatever eCW returns into one predictable pair:
--
--    Result        - the parsed resource or Bundle
--    nil, Err      - Err is { code=, message= } plus optional context
--
-- Two eCW behaviours are worth knowing, because both look like success at the
-- HTTP layer and are reported as failures here:
--
--   * An OperationOutcome resource can arrive with HTTP 200. eCW uses it for
...
-- Interpret a response with no body. A 2xx here is eCW's create success.
local function readEmptyBody(Response)
   ...
   return nil, {
      ...
      message = 'eCW returned HTTP ' .. tostring(Response.code) .. ' with an empty body',
   }
end
...
local function readBody(Response)
   ...
   if not Ok then
      return nil, {
         code      = 'PARSE_ERROR',
         message   = 'eCW response was not valid JSON',
   ...
   if isOperationOutcome(Parsed) then
      return nil, {
         ...
         message   = outcomeText(Parsed) or 'eCW returned an OperationOutcome',
         http_code = Response.code,
         outcome   = Parsed,
      }
   end
   ...
   if Response.code < 200 or Response.code >= 300 then
      return nil, {
         code      = 'HTTP_' .. tostring(Response.code),
         message   = 'eCW returned HTTP ' .. tostring(Response.code),
         http_code = Response.code,
         body      = Parsed,
      }
   end
...
--   T.api        - path below the base URL, e.g. 'Patient/abc123'
--   T.url        - full URL override (used by bulk download)
--   T.parameters - query table for GET-like verbs, JSON body for the rest
--   T.headers    - extra headers
--   T.timeout    - per-request timeout override
...
function M.request(Client, T)
   ...
   if not SendRequest then
      error("ecw_fhir_http.request: unsupported HTTP method '" .. Method .. "'")
   end
   ...
   local Url = T.url or (Client.base_url .. tostring(T.api or ''))

   local Request = {
      url       = Url,
      headers   = Headers,
      timeout   = T.timeout or Client.timeout,
   ...
   linkiir.log.debug('ecw_fhir ' .. Method:upper() .. ' ' .. Request.url)
